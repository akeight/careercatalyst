...
export default function KanbanBoardApp() {
    ...
    const handleDragEnd = (event: DragEndEvent) => {
        ...
        if (activeColumnId === overColumnId) {
            ...
            const oldIndex = column.items.findIndex((item) => item === activeItemId);
            const newIndex = column.items.findIndex((item) => item === overItemId);
            ...
            const newPosition = overItemId
                ? columns[overColumnId].items.findIndex((item) => item === overItemId)
        ...
        >
            <div className="grid grid-cols-1 md:grid-cols-3 gap-2 p-6">
                {Object.values(columns).map((column) => (
                    <SortableContext
                        key={column.id}
                        items={column.items.map((item) => `${column.id}:${item}`)}
